...
(직접 타이핑) 만약 `print(space)`를 한다면, 모든 element가 달라야 한다는 제약조건에 따라 중복되는 element는 알아서 제거하는 것을 볼 수 있음. 또한 set은 순서가 상관 없는 자료형이기에 순서가 바뀔 수도 있음.
...
딕셔너리는 순서가 있고 mutable한 element를 중복 없이 저장하는데, 키(key)와 값(value)의 쌍으로 데이터를 저장하는 자료형입니다.
...
`del grade_dict["영희"]`를 하면 영희라는 키 값이랑 value가 함께 삭제가 돼요. 이 또한 마찬가지로 매우 빠른 속도를 가져요.
